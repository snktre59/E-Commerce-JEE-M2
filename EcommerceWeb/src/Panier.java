	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		for (Article liste : panier) {
			if (liste.getStock() < Integer.valueOf(request.getParameter(String.valueOf(liste.getId())))) {
				 ...
				 */
				request.setAttribute("errorStock", "Probl�me de stock");
				getServletContext().getRequestDispatcher("/panier.jsp").forward(request, response);
			}
			total+=liste.getPrix();
		}
		if (hs.getAttribute("utilisateur") == null) {
			ok=false;
			request.setAttribute("errorUser", "Vous devez �tre connect�.");
			getServletContext().getRequestDispatcher("/panier.jsp").forward(request, response);
		}
		
		if (ok) {
			Utilisateur user = (Utilisateur) hs.getAttribute("utilisateur");
			Facture f =facture.addFacture(user, total);
			for (Article liste : panier) {
				 	
					
					commande.addCommande(liste, user.getId(), liste.getPrix(), Integer.valueOf(request.getParameter(String.valueOf(liste.getId()))), f);
